namespace FlexiblePathfindingSystem3D
{
    ...
    public class NavMeshLinksGenerator : MonoBehaviour
    {
        ...
        public void CreateLinks()
        {
            ...
            if (generatedLinksGroup == null)
            {
                generatedLinksGroup = new GameObject("AllLinksGroup").transform;
            }

            if (standardLinkPrefab == null)
            {
                Debug.LogWarning("Links could not be generated as StandardLinkPrefab has not been assigned. " +
                    "\nPress AutoAssign button and check the prefab has been assigned in the appropriate box.");
                return;
            }
            ...
            try
            {
                foreach (Edge edge in edges)
                {
                    foreach (Edge targetEdge in edges)
                    {
                        ...
                        if (LinkExists(edge.connectionPoint[startPointIndex], targetEdge.connectionPoint[endPointIndex]))
                        {
                            continue;
                        }
                        ...
                        {
                            // Instantiate the link prefab at the start position
                            Transform linkObject = Instantiate(
                                standardLinkPrefab.transform,
                                edge.connectionPoint[startPointIndex],
                                Quaternion.identity
                            );

                            // Get the NavMeshLink component
                            var link = linkObject.GetComponent<NavMeshLink>();

                            // Create a child GameObject for the start position (if not already present)
                            if (linkObject.Find("StartPosition") == null)
                            {
                                GameObject startPosObj = new GameObject("StartPosition");
                                startPosObj.transform.SetParent(linkObject);
                                startPosObj.transform.localPosition = Vector3.zero;
                            }

                            // Create a child GameObject for the end position (if not already present)
                            if (linkObject.Find("EndPosition") == null)
                            {
                                GameObject endPosObj = new GameObject("EndPosition");
                                endPosObj.transform.SetParent(linkObject);
                            }

                            // Set the end position in world space
                            Transform endPosTransform = linkObject.Find("EndPosition").transform;
                            endPosTransform.position = targetEdge.connectionPoint[endPointIndex];

                            // Assign the transforms to the NavMeshLink
                            link.startTransform = linkObject.Find("StartPosition").transform;
                            link.endTransform = endPosTransform;
        ...
        public void AddDropDownLink(Edge edge)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                Vector3 checkDirection = rotation * edge.falloffDirection.normalized;

                if (Physics.Raycast(
                    edge.falloffPoint[0],
                    checkDirection + (dropDownSteepnessModifier * Vector3.down),
                    out RaycastHit hit,
                    maxDropDownLinkDistance))
                {
                    if (LinkExists(edge.connectionPoint[0], hit.point))
                        return;

                    // Instantiate link at start position
                    Transform linkObject = Instantiate(
                        dropDownLinkPrefab.transform,
                        edge.connectionPoint[0],
                        Quaternion.identity
                    );

                    // Get NavMeshLink component
                    var link = linkObject.GetComponent<NavMeshLink>();

                    // Create start transform if missing
                    if (linkObject.Find("StartPosition") == null)
                    {
                        GameObject startPosObj = new GameObject("StartPosition");
                        startPosObj.transform.SetParent(linkObject);
                    ...
                    // Create end transform if missing
                    if (linkObject.Find("EndPosition") == null)
                    {
                        GameObject endPosObj = new GameObject("EndPosition");
                        endPosObj.transform.SetParent(linkObject);
                    ...
                    // Set end position to the raycast hit point
                    Transform endPosTransform = linkObject.Find("EndPosition").transform;
                    endPosTransform.position = hit.point;

                    // Assign transforms to NavMeshLink
                    link.startTransform = linkObject.Find("StartPosition").transform;
                    link.endTransform = endPosTransform;
                    link.UpdateLink();
